...
import { BookOpen, Clock, Search, CheckCircle, GraduationCap, Star, Play } from "lucide-react";
...
const CoursesOverview = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: coursesData,
        error: coursesError
      } = await supabase.from('courses').select('*').eq('category', 'course').order('created_at', {
        ascending: false
      });
  ...
  return <div className="min-h-screen bg-black flex relative overflow-hidden">
      ...
      <div className={`fixed left-0 top-0 h-screen z-40 transition-all duration-300 ${sidebarCollapsed ? 'w-16 sm:w-20' : 'w-64 sm:w-72'}`}>
        <Sidebar selectedSheet="courses" onSheetChange={() => {}} collapsed={sidebarCollapsed} onToggleCollapse={() => setSidebarCollapsed(!sidebarCollapsed)} />
      </div>
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="relative">
                ...
                <Input placeholder="Search courses by topic, technology, or difficulty..." value={filterQuery} onChange={e => setFilterQuery(e.target.value)} className="pl-12 bg-black border-gray-800 text-white placeholder-gray-500 h-12 text-lg focus:border-blue-500" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              const cardColors = ['from-green-600 to-emerald-800', 'from-blue-600 to-cyan-700', 'from-teal-600 to-green-800', 'from-emerald-500 to-teal-600'];
