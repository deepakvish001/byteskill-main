...
import { Users, Clock, Search, Target, CheckCircle, MessageCircle, Star, BookOpen, Play } from "lucide-react";
...
const InterviewPrepPage = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const {
        data: coursesData,
        error: coursesError
      } = await supabase.from('courses').select('*').eq('category', 'interview-prep').order('created_at', {
        ascending: false
      });
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      ...
      default:
        return 'bg-gray-900/80 text-gray-300 border-gray-700';
    }
  };
  ...
  return <div className="min-h-screen bg-black flex relative overflow-hidden">
      ...
      <div className={`fixed left-0 top-0 h-screen z-40 transition-all duration-300 ${sidebarCollapsed ? 'w-16 sm:w-20' : 'w-64 sm:w-72'}`}>
        <Sidebar selectedSheet="interview-prep" onSheetChange={() => {}} collapsed={sidebarCollapsed} onToggleCollapse={() => setSidebarCollapsed(!sidebarCollapsed)} />
      </div>
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="relative">
                ...
                <Input placeholder="Search interview prep courses by company, topic, or difficulty..." value={filterQuery} onChange={e => setFilterQuery(e.target.value)} className="pl-12 bg-black border-gray-800 text-white placeholder-gray-500 h-12 text-lg focus:border-blue-500" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              const cardColors = ['from-purple-600 to-purple-800', 'from-pink-600 to-rose-700', 'from-violet-600 to-purple-800', 'from-indigo-500 to-purple-600'];
              ...
              return <Card key={course.id} className={`group bg-gradient-to-br ${cardColor} border-0 text-white relative overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl cursor-pointer`}>
                    ...
                    <CardContent className="pt-0 bg-transparent relative z-10">
                      ...
                      <div className="space-y-2">
                        <Button onClick={() => navigate(`/course/${course.course_id}`)} className="w-full bg-white/20 hover:bg-white/30 text-white border-0 font-medium py-2 transition-all duration-200">
